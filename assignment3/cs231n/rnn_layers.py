...
def word_embedding_forward(x, W):
    ...
    给出词索引,给出各个时间对应的输入词,给出该词对应的向量,构建输入矩阵
    """
    out, cache = None, None
    ##############################################################################
...
def word_embedding_backward(dout, cache):
    ...
    for n in xrange(N):
        for t in xrange(T):
            dW[x[n, t]] += dout[n, t, :]  # 对权值的梯度为各时间对对应的梯度的总和
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cell = np.zeros([N, H])
    for i in xrange(T):
        hidden, cell, subcache = lstm_step_forward(x[:, i, :], hidden, cell, Wx, Wh, b)
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    return loss, dx
>>>>>>> 7326a5237d8fa16848961a4de5206e036e5a936c
